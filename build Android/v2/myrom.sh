...
rom_jack(){
	...
	./prebuilts/misc/linux-x86/ccache/ccache -M 50G $1>/dev/null
}
...
rom_atualizar(){
	...
	#rom_scripts_build
	#rom_device
  #cd ~/$ROM/frameworks/native

	#git fetch https://review.gzospgzr.com/GZOSP/frameworks_native refs/changes/79/2179/1 && git cherry-pick FETCH_HEAD
	#cd ~/$ROM
...
rom_make(){
	...
	rom_ccache
	#pwd
	#if [ "/$path/$ROM" = 'xtended' ] ; then
	#	pasta="rr"
	#	nomeDev="rr"
	#. build/envsetup.sh && brunch lineage_sanders-userdebug -j16
		# . build/envsetup.sh && lunch aosp_sanders-userdebug &&  mka aex -j32
	. build/envsetup.sh &&   lunch xtended_sanders-userdebug  && make xtended -j16
...
rom_limpar(){
	outdir="/home/j144df/xtended/out/target/product/sanders"
...
rom_ccache(){
	export USE_CCACHE=1
	export CCACHE_DIR=$HOME/.ccache-pie/$ROM
